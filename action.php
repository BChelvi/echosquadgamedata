...
require './credentials.php';
	...
	switch($_POST["MessageType"])
	{
		case "RoomLogin":
			...
			if ($preparedQuery->execute()) {
				...
				if ($preparedQuery->execute()) {
					...
					echo "Error: Failed query for new room session (" . $query . ")";
				}				
			}
			else {
				echo "Error: Failed to insert new room sesion (" . $query . ")";
			}
			break;
		...
		case "EraseEndDate":
			$query = "UPDATE gamesession SET EndDate = NULL WHERE gamesessionId=" . $_POST["Id"];
			$preparedQuery = $db->prepare($query);
			$preparedQuery->execute();
			
		default:
			echo "Error: Unkonwn message type (" . $_POST["MessageType"] . ")";
